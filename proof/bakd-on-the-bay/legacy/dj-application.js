...
document.getElementById('djForm').addEventListener('submit', function (e) {
  ...
  // Send every field on the form. The applications function stores them under
  // the "dj" type and the admin dashboard reviews and approves/rejects each one.
  var fields = Object.fromEntries(new FormData(this).entries());

  fetch('/.netlify/functions/applications', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ type: 'dj', fields: fields })
  })
    .then(function (res) {
    ...
    .catch(function (err) {
      console.error('DJ application submission error:', err);
