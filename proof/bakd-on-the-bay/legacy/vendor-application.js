...
document.getElementById('vendorForm').addEventListener('submit', function (e) {
  ...
  // Send every field on the form. The applications function stores them and the
  // admin dashboard reviews and approves/rejects each submission.
  var fields = Object.fromEntries(new FormData(this).entries());

  fetch('/.netlify/functions/applications', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ type: 'vendor', fields: fields })
  })
    .then(function (res) {
