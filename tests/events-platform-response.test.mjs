...
test('events platform response exposes the contract registry without database setup', async () => {
  ...
  assert.ok(Array.isArray(body.contracts.notifications.notificationTypes))
  assert.ok(Array.isArray(body.contracts.moderationTrustSafety.caseTypes))
  ...
  assert.ok(Array.isArray(body.contracts.scheduling.entryTypes))
  assert.ok(Array.isArray(body.contracts.placesMaps.placeTypes))
  assert.ok(Array.isArray(body.contracts.contentCms.contentTypes))
  ...
  assert.ok(Array.isArray(body.contracts.commercePayments.recordTypes))
  assert.ok(Array.isArray(body.contracts.integrationsWebhooks.integrationTypes))
  assert.ok(Array.isArray(body.contracts.operationsAudit.recordTypes))
  ...
  assert.ok(Array.isArray(body.contracts.themes.recordTypes))
  assert.ok(Array.isArray(body.contracts.adminOs.workspaceTypes))
  assert.ok(Array.isArray(body.contracts.authAccessControl.recordTypes))
  assert.ok(Array.isArray(body.contracts.dataPortability.exportScopes))
  assert.ok(Array.isArray(body.contracts.imports.targets))
  assert.ok(Array.isArray(body.contracts.migrations.changeTypes))
  assert.ok(Array.isArray(body.contracts.releaseGates.gateAreas))
  ...
  assert.equal(body.contractsSummary.notificationTypeCount, body.contracts.notifications.notificationTypes.length)
  assert.equal(
    body.contractsSummary.moderationCaseTypeCount,
    body.contracts.moderationTrustSafety.caseTypes.length,
  )
  ...
  assert.equal(body.contractsSummary.schedulingEntryTypeCount, body.contracts.scheduling.entryTypes.length)
  assert.equal(body.contractsSummary.placeTypeCount, body.contracts.placesMaps.placeTypes.length)
  assert.equal(body.contractsSummary.contentTypeCount, body.contracts.contentCms.contentTypes.length)
  ...
  assert.equal(body.contractsSummary.commerceRecordTypeCount, body.contracts.commercePayments.recordTypes.length)
  assert.equal(
    body.contractsSummary.integrationTypeCount,
    body.contracts.integrationsWebhooks.integrationTypes.length,
  )
  assert.equal(body.contractsSummary.operationsRecordTypeCount, body.contracts.operationsAudit.recordTypes.length)
  ...
  assert.equal(body.contractsSummary.themeRecordTypeCount, body.contracts.themes.recordTypes.length)
  assert.equal(body.contractsSummary.adminWorkspaceTypeCount, body.contracts.adminOs.workspaceTypes.length)
  assert.equal(body.contractsSummary.authRecordTypeCount, body.contracts.authAccessControl.recordTypes.length)
  assert.equal(body.contractsSummary.dataPortabilityScopeCount, body.contracts.dataPortability.exportScopes.length)
  assert.equal(body.contractsSummary.importTargetCount, body.contracts.imports.targets.length)
  assert.equal(body.contractsSummary.migrationChangeTypeCount, body.contracts.migrations.changeTypes.length)
  assert.equal(body.contractsSummary.releaseGateAreaCount, body.contracts.releaseGates.gateAreas.length)
})
