...
const paymentController = read('assets/js/payment-destination-controller.mjs')
const createStyles = read('assets/css/create.css')
...
test('payment destination UI separates lead forms, usernames, and secure links', () => {
  assert.match(builder, /Action destination/)
  assert.match(builder, /payment-destination-controller\.mjs/)
  assert.match(builder, /id="launchButton" type="button"/)
  assert.match(paymentController, /Cash App \$Cashtag/)
  assert.match(paymentController, /Beslyfe will create and connect an internal lead form/)
  assert.match(paymentController, /Paste the complete secure https:\/\/ link/)
  assert.match(paymentController, /prepared\.kind === 'built-in'/)
  assert.match(paymentController, /if \(submitting\) return/)
  assert.match(paymentController, /Retry growth action/)
  assert.match(createStyles, /#salesDestinationField\[hidden\]/)
  assert.match(createStyles, /#nextButton\[hidden\]/)
  assert.match(createStyles, /@media\(max-width:600px\)[\s\S]*#launchButton\{width:100%/)
})
